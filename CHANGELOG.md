...
- Keep Search and Favorites as smaller background-free icons fixed four pixels inside the conversation's selected upper corner. They mirror only when the side changes and no longer follow timeline edge or vertical offsets. Favorites filters the official timeline in place without a count or popup. Give every idle official dash the same shortest `8px` length regardless of its loaded state; the transcript's current-Turn dash retains a softer official highlight without growing. Restore a restrained symmetric Codex `30 / 24 / 18 / 12px` staircase around the hovered or keyboard-focused official dash, with the center as the only primary highlight. Keep the enhanced viewport wide enough to avoid clipping by the official 28px scroller. Pointer and wheel retargeting now morph staircase dashes continuously from their current lengths instead of restarting a fixed peak animation on each new center. Entering or leaving the interactive state hands length, color, and opacity highlight states between the real current-Turn and pointer-center dashes over about `180ms`; it no longer overlays a separate dash that travels along the rail, and ordinary dashes outside the staircase stay unchanged. Reduced-motion mode removes these transitions. Wheel scrolling retargets the staircase and preview to the Turn under the stationary pointer. Enrich the preview with timestamp and one compact performance/Token stats block, while keeping the smaller Favorite and Branch actions inline at the far right of the Turn row. Jumps use alpha.3's `loadThrough(seq)`, branches use `sessions.fork({ atSeq })`, and the Host index exposes both performance data and a separate branch anchor for each completed Turn.
- Preserve the useful personalization options—enable switch, side, edge offset, center offset, marker spacing, and recent-Turn count—while applying layout geometry to the official rail. Remove the post-jump arrival flash, its CSS animation, persisted setting, and settings control. Disabling the plugin restores the official navigator instead of hiding it.
- Keep the standalone disclosure-card behavior in Plugin configuration while making range sliders responsive: drag values stay local, geometry previews are coalesced to at most one update per animation frame, and release, blur, or keyboard completion persists only the final value to the Host. Slider values retain overridden/reset affordances, and left/right placement mirrors the official dashes, preview popover, and inward edge offset as one unit.
- Make official-layout restoration idempotent. Turning enhancements off now restores the alpha.3 rail once and leaves its `--turn-natural-height` intact instead of a second cleanup collapsing the official navigator to zero height.
- Replace the historical UI-corpus release checks with alpha.3 integration contracts covering official ownership, public slots, semantic controls, settings persistence, long-session indexing, jump anchors, branch anchors, and fail-open startup.

## 0.5.6 - 2026-09-01

- Fix [#9](https://github.com/Wine-Red/dsh-codex-timeline/issues/9): support DSH `0.1.2-alpha.3` without blocking Host or browser startup. The Host adapter now uses the literal settings namespace accepted by the new settings package instead of importing the removed `settingsNamespace` helper, and the browser artifact no longer resolves the removed `@deepseek-ai/dsh-client-runtime/client` module.
- Retarget the verified dependency, client-injection, installer, and compatibility metadata to the alpha.3 package split while preserving the public header-action slot, Session binding API, chat-flow anchors, and additive ownership model.
