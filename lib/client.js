window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		const zh = {
			...
			"settings.showOnRightHint": "关闭时将增强导航显示在左侧。",
		...
		const en = {
			...
			"settings.showOnRightHint": "When off, show the enhanced rail on the left.",
...
nav[data-dsh-navx-replaced=true]{visibility:hidden!important;pointer-events:none!important}
nav.dsh-navx-rail{--turn-rail-band:calc(var(--dsh-conversation-viewport-height,100dvh) - var(--dsh-composer-height,152px));position:absolute;top:calc(var(--turn-rail-band) / 2);transform:translateY(-50%);pointer-events:auto;z-index:1}
nav.dsh-navx-rail[data-dsh-navigation-side=left]{left:calc(12px + var(--dsh-navx-edge-offset,0px));right:auto}
nav.dsh-navx-rail[data-dsh-navigation-side=right]{right:calc(12px + var(--dsh-navx-edge-offset,0px));left:auto}
.dsh-navx-railScroll{height:100%;overflow-y:auto;overscroll-behavior:contain;scrollbar-width:none}
.dsh-navx-railScroll::-webkit-scrollbar{display:none}
.dsh-navx-railTrack{position:relative;height:var(--turn-natural-height)}
.dsh-navx-railPosition{position:absolute;top:calc(1px + var(--turn-natural-position));left:0;right:0;height:10px}
.dsh-navx-railPosition>button{position:absolute;inset:0;width:36px;height:10px;border:0;padding:0;background:transparent;cursor:pointer}
.dsh-navx-railPosition>button::before{content:"";position:absolute;top:50%;height:2px;border-radius:2px;transform:translateY(-50%)}
.dsh-navx-railPosition>button:focus-visible{outline:1px solid var(--dsw-alias-state-business-primary);outline-offset:1px;border-radius:3px}
...
`;
		...
		function officialNavigator() {
			// rc7 moved the rail outside the chat-flow's immediate scroll parent.
			for (const button of document.querySelectorAll('nav button[data-index]')) {
				const nav = button.closest("nav");
				if (nav !== null && !nav.classList.contains("dsh-navx-rail")) return nav;
			}
			return null;
		}

		function rc7Navigator(source, items, navigate, t) {
			if (source === null || items.length === 0) return null;
			let nav = source.parentElement.querySelector(".dsh-navx-rail");
			if (nav === null) {
				nav = document.createElement("nav");
				nav.className = "dsh-navx-rail";
				nav.setAttribute("aria-label", source.getAttribute("aria-label") ?? "Timeline");
				const scroller = document.createElement("div");
				scroller.className = "dsh-navx-railScroll";
				const track = document.createElement("div");
				track.className = "dsh-navx-railTrack";
				scroller.append(track);
				nav.append(scroller);
				source.parentElement.append(nav);
				nav.dshSource = source;
				nav.dshSourceInert = source.inert;
				source.dataset.dshNavxReplaced = "true";
				source.inert = true;
			}
			const track = nav.firstElementChild.firstElementChild;
			const key = items.map((item) => item.turn).join(",");
			if (nav.dataset.turns !== key) {
				track.replaceChildren(...items.map((item) => {
					const position = document.createElement("div");
					position.className = "dsh-navx-railPosition";
					const button = document.createElement("button");
					button.type = "button";
					button.dataset.turn = String(item.turn);
					button.setAttribute("aria-label", t("jump", { turn: item.turn }));
					button.onclick = () => nav.dshNavigate(item);
					position.append(button);
					return position;
				}));
				nav.dataset.turns = key;
			}
			nav.dshNavigate = navigate;
			const activeIndex = Number(source.querySelector('[aria-current="true"]')?.dataset.index ?? -1);
			const activeTurn = items[activeIndex]?.turn;
			for (const button of track.querySelectorAll("button")) {
				const active = button.dataset.turn === String(activeTurn);
				if (active && button.getAttribute("aria-current") !== "true") {
					button.setAttribute("aria-current", "true");
					if (!nav.matches(":hover") && !nav.contains(document.activeElement)) {
						requestAnimationFrame(() => {
							if (button.isConnected) nav.firstElementChild.scrollTop = button.parentElement.offsetTop - nav.clientHeight / 2;
						});
					}
		...
		function restoreOfficialNavigator(nav) {
			if (nav.dshSource) {
				delete nav.dshSource.dataset.dshNavxReplaced;
				nav.dshSource.inert = nav.dshSourceInert;
				nav.remove();
				return;
			}
		...
		function enhanceOfficialNavigator(preferences, favoritesOnly, items, navigate, t) {
			const nav = preferences.enabled ? rc7Navigator(officialNavigator(), items, navigate, t) : null;
		...
		function previewOfficialRangePreference(preferences, settingKey, value) {
			const nav = document.querySelector(".dsh-navx-rail");
		...
		function useOfficialNavigatorEnhancement(preferences, favoritesOnly, items, navigate, t) {
			...
			useEffect(() => {
				...
				const apply = () => {
					frame = null;
					const next = enhanceOfficialNavigator(preferences, favoritesOnly, items, navigate, t);
				...
				apply();
				const observer = typeof MutationObserver === "undefined" || document.body === null ? null : new MutationObserver((changes) => {
					if (changes.some((change) => !change.target.closest?.(".dsh-navx-rail"))) schedule();
				});
			...
			}, [preferences.enabled, preferences.side, preferences.leftOffset, preferences.centerOffset, preferences.markerSpacing, preferences.recentTurns, preferences.favorites, favoritesOnly, items, navigate, t]);
			return surface;
		}
		...
		function NavigationAction({ sessionId, preferences, setPreference, loadThrough, forkAt, t }) {
			...
			const surface = useOfficialNavigatorEnhancement(value, favoritesOnly, indexItems, navigate, t);
		...
		function apply(ctx) {
			...
			safeSlot(ctx, "settings.plugins.tab", () => ctx.slots.register({
