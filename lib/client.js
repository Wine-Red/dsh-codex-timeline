	...
	factory: (require) => {
		...
		*/
		function buildTurnNavigationIndex(chat, pending, running) {
			...
			for (const turnNumber of chat.timeline.turnOrder) {
				...
				const anchorKey = turnKeys.find((key) => chat.nodes.get(key)?.kind === "user");
				if (anchorKey === void 0) continue;
				const viewNode = chat.nodes.get(anchorKey);
				if (viewNode?.kind !== "user") continue;
				const user = viewNode;
		...
		function timeLabel(time) {
			...
			return new Intl.DateTimeFormat(void 0, sameDay ? {
				hour: "2-digit",
				minute: "2-digit"
			} : {
				month: "2-digit",
		...
		function TurnNavigation({ sessionId, useSession, usePreferences, layout, hasMore, loadingOlder, scrollToAnchor, trackAnchors, loadOlder, forkAt, setPreference, t }) {
			...
			const autoLoadTimerRef = (0, react.useRef)(null);
			...
			(0, react.useEffect)(() => {
				...
				if (!preferences.enabled || !preferences.autoLoadAll || !hasMore || loadingOlder) return;
				autoLoadTimerRef.current = window.setTimeout(() => {
					autoLoadTimerRef.current = null;
					Promise.resolve(loadOlderRef.current()).catch((error) => {
						console.error("dsh-codex-timeline: failed to auto-load older history:", error);
					});
				}, 80);
				return () => {
					if (autoLoadTimerRef.current !== null) {
			...
			}, [preferences.enabled, preferences.autoLoadAll, hasMore, loadingOlder, sessionId]);
			...
			const autoHidden = !favoritesOnly && items.length < 3 && !hasMore;
			...
			const searchResults = (0, react.useMemo)(() => normalizedSearchQuery === "" ? [] : visibleItems.filter((item) => item.searchText.includes(normalizedSearchQuery)), [visibleItems, normalizedSearchQuery]);
			...
			(0, react.useEffect)(() => {
				setSearchIndex((index) => Math.max(0, Math.min(index, searchResults.length - 1)));
			...
			}, [searchIndex, searchOpen, searchResults]);
			(0, react.useEffect)(() => {
				trackAnchors(preferences.enabled && !autoHidden ? visibleItems.flatMap((item) => item.anchorKey === void 0 ? [] : [item.anchorKey]) : []);
				return () => {
					trackAnchors([]);
				};
			}, [
			...
			(0, react.useEffect)(() => {
				...
				const item = items.find((candidate) => candidate.id === pendingJumpId);
				if (item === void 0) {
					setPendingJumpId(null);
					return;
				}
				if (item.anchorKey !== void 0 && positions.has(item.anchorKey)) {
					scrollToAnchor(item.anchorKey, prefersReducedMotion() ? "auto" : "smooth");
					setPendingJumpId(null);
					return;
				}
				if (!hasMore) {
			...
			}, [pendingJumpId, items, positions, hasMore, loadingOlder, loadOlder, scrollToAnchor]);
			...
			if (!preferences.enabled || autoHidden || !favoritesOnly && located.length === 0 && !hasMore) return null;
			return /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
					...
					railVisible && /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("nav", {
						...
						children: [
							...
							hasMore && /* @__PURE__ */ (0, react_jsx_runtime.jsx)("button", {
									...
									title: loadingOlder ? t("timeline.loadingEarlier") : t("timeline.earlier"),
									onClick: loadOlder,
									children: /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("svg", {
										width: 14,
										height: 14,
							...
							searchOpen && /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
								...
								children: [/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
									...
									role: "status",
									"aria-live": "polite",
									children: normalizedSearchQuery === "" ? t("timeline.search.hint") : searchResults.length > 0 ? t("timeline.search.count", { count: searchResults.length }) : t(hasMore ? "timeline.search.noneEarlier" : "timeline.search.none")
								}), searchResults.length > 0 && /* @__PURE__ */ (0, react_jsx_runtime.jsx)("div", {
									className: TurnNavigation_module_css_default.searchResults,
					children: searchResults.map((item, index) => /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("button", {
						ref: (element) => {
						...
						type: "button",
							...
							/* @__PURE__ */ (0, react_jsx_runtime.jsx)("div", {
								...
								style: { top: hasMore ? 76 : 52 },
								children: located.map(({ item, anchor }, index) => {
									const current = item.id === activeId;
									const disclosed = item.id === disclosedId;
									const showCurrent = current && hoveredId === null;
									const sparseMinor = preferences.density === "sparse" && index % 4 !== 0 && !showCurrent && !disclosed;
									return /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
										key: item.id,
										className: TurnNavigation_module_css_default.markerSlot,
										style: { "--turn-position": anchor.position },
										onMouseEnter: () => {
										...
										children: [/* @__PURE__ */ (0, react_jsx_runtime.jsx)("div", {
											...
											style: { "--turn-position": anchor.position },
											"aria-label": markerLabel(item, t),
											"aria-current": current ? "location" : void 0,
											tabIndex: (focusedId ?? activeId ?? visibleItems[0]?.id) === item.id ? 0 : -1,
											onClick: (event) => {
												jump(item);
						...
						children: /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("section", {
							...
							children: [
								...
								/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
									...
									className: TurnNavigation_module_css_default.searchStatus,
									role: "status",
									children: searchResults.length > 0 ? t("timeline.search.count", { count: searchResults.length }) : t(hasMore ? "timeline.search.noneEarlier" : "timeline.search.none")
								...
								children: (normalizedSearchQuery === "" ? visibleItems : searchResults).map((item) => {
									const metrics = performanceParts(item, t);
									return /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("button", {
										type: "button",
					...
					open ? (0, react_jsx_runtime.jsxs)("div", {
						...
						children: [
							...
							(0, react_jsx_runtime.jsxs)("div", {
								className: SettingsRow_module_css_default.field,
								children: [(0, react_jsx_runtime.jsx)(Switch, {
									label: t("settings.autoLoadAll"),
									checked: settings.autoLoadAll,
									onChange: (value) => {
										setPreference("autoLoadAll", value);
									}
								}), (0, react_jsx_runtime.jsx)("p", {
									className: SettingsRow_module_css_default.hint,
									children: t("settings.autoLoadAllHint")
								})]
							}),
		...
		*/
		var TurnNavigationPreferences = class {
			...
			adopt(value) {
				const next = Object.freeze({ ...DEFAULT_TURN_NAVIGATION_SETTINGS, ...value });
				if (next.enabled === this.snapshot.enabled && next.autoLoadAll === this.snapshot.autoLoadAll && next.favorites.length === this.snapshot.favorites.length && next.favorites.every((id, index) => id === this.snapshot.favorites[index]) && next.defaultVisible === this.snapshot.defaultVisible && next.density === this.snapshot.density && next.followHighlight === this.snapshot.followHighlight && next.leftOffset === this.snapshot.leftOffset && next.centerOffset === this.snapshot.centerOffset && next.markerSpacing === this.snapshot.markerSpacing) return;
				this.snapshot = next;
				this.publish();
			}
			publish() {
		...
		const zh = {
			...
			"timeline.search.hint": "输入关键词搜索已加载内容",
			...
			"settings.enabled": "启用",
			"settings.autoLoadAll": "自动加载全部历史",
			"settings.autoLoadAllHint": "每 80 ms 继续加载一页更早历史，直到完整正文全部展开。",
		...
		const en = {
			...
			"timeline.search.hint": "Search loaded conversation content",
			...
			"settings.autoLoadAll": "Automatically load full history",
			"settings.autoLoadAllHint": "Load another older-history page every 80 ms until the complete transcript is expanded.",
