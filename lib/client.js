	...
	factory: (require) => {
		...
		*/
		function useChatNavigationBridge({ sessionId, listRef, columnRef, hasMore, loadingOlder, loadOlder, toBottom, forkAt }) {
			...
			const scheduleMeasure = (0, react.useCallback)((full = false) => {
				if (trackedKeys.current.size === 0) return;
			...
			return {
				...
				owner: {
					...
					trackAnchors: (0, react.useCallback)((keys) => {
		...
		*/
		...
		if (typeof document !== "undefined" && document.querySelector("style[data-plugin-css=" + JSON.stringify(tagId$1) + "]") === null) {
			...
			const jumpTagId = "dsh-codex-timeline/JumpStatus.css";
			if (typeof document !== "undefined" && document.querySelector("style[data-plugin-css=" + JSON.stringify(jumpTagId) + "]") === null) {
				const jumpTag = document.createElement("style");
				jumpTag.dataset.plugin = "dsh-codex-timeline";
				jumpTag.dataset.pluginCss = jumpTagId;
				jumpTag.textContent = ".dsh-tl-jumpStatus{position:absolute;bottom:2px;left:2px;right:2px;z-index:9;color:var(--dsw-alias-label-secondary);background:var(--dsw-alias-bg-overlay);border:1px solid var(--dsw-alias-border-l1);border-radius:6px;padding:1px 4px;font-size:11px;line-height:16px;text-align:center;white-space:nowrap;overflow:hidden;text-overflow:ellipsis}";
				document.head.appendChild(jumpTag);
			}
		...
		/** Rail chip text for the observable jump state machine. */
		function jumpStatusText(stage, pendingJumpId, t) {
			const turn = String(pendingJumpId ?? "").replace("turn:", "");
			switch (stage) {
				case "jump-paging": return t("timeline.jump.paging", { turn });
				case "jump-locating": return t("timeline.jump.locating", { turn });
				case "jump-done": return t("timeline.jump.done", { turn });
				default: return t("timeline.jump.failed", { turn });
			}
		}
		...
		function TurnNavigation({ sessionId, useSession, usePreferences, layout, hasMore, loadingOlder, scrollToAnchor, trackAnchors, loadOlder, forkAt, setPreference, t }) {
			...
			// content) feeds the rail's recent-20 ticks; null means unavailable.
			...
			// The rail shows the RECENT turns only (newest 20), at the user's
			// own marker-spacing setting — same look as the original rail, no
			// compression. Older turns stay reachable through 加载更早 / search.
			const railItems = (0, react.useMemo)(() => railItemsAll.slice(-(preferences.recentTurns ?? 25)), [railItemsAll, preferences.recentTurns]);
			...
			const railGap = Math.max(
				2,
				Math.min(
					preferences.markerSpacing ?? 10,
					(railTrackHeight - 12) / Math.max(1, railItems.length),
				),
			);
			const railGroupHeight = railItems.length * railGap;
			const railStartOffset = Math.max(0, (railTrackHeight - railGroupHeight) / 2);
			...
			(0, react.useEffect)(() => {
				trackAnchors(preferences.enabled ? visibleItems.flatMap((item) => item.anchorKey === void 0 ? [] : [item.anchorKey]) : []);
				return () => {
					trackAnchors([]);
				};
			}, [
			...
			// Observable jump state machine: the rail shows which stage a jump
			// is in, so a silent stall becomes a visible, reportable stage.
			const [jumpStage, setJumpStage] = (0, react.useState)(null);
			const jumpStageTimerRef = (0, react.useRef)(0);
			const clearJumpStage = (0, react.useCallback)(() => {
				if (jumpStageTimerRef.current !== 0) window.clearTimeout(jumpStageTimerRef.current);
				jumpStageTimerRef.current = window.setTimeout(() => setJumpStage(null), 4000);
			}, []);
			(0, react.useEffect)(() => {
				...
				if (item.anchorKey !== void 0) {
					...
					if (positions.has(item.anchorKey)) {
						...
						setJumpStage(landed ? "jump-done" : "jump-failed");
						clearJumpStage();
						setPendingJumpId(null);
						return;
					}
					...
					setJumpStage("jump-locating");
					const timer = window.setTimeout(() => {
						trackAnchors(preferences.enabled ? visibleItems.flatMap((candidate) => candidate.anchorKey === void 0 ? [] : [candidate.anchorKey]) : []);
			...
			}, [pendingJumpId, allItems, positions, hasMore, loadingOlder, loadOlder, scrollToAnchor, trackAnchors, preferences.enabled, visibleItems, jumpMeasureTick, clearJumpStage]);
			...
			const moveFocus = (0, react.useCallback)((from, delta) => {
				const index = visibleItems.findIndex((item) => item.id === from);
				if (index < 0) return;
				const next = visibleItems[Math.max(0, Math.min(visibleItems.length - 1, index + delta))];
			...
			const onMarkerKeyDown = (event, item) => {
				if (event.key === "ArrowUp" || event.key === "ArrowLeft") {
					...
					if (first !== void 0) moveFocus(first.id, -visibleItems.length);
				} else if (event.key === "End") {
					event.preventDefault();
					const last = visibleItems.at(-1);
					if (last !== void 0) moveFocus(last.id, visibleItems.length);
				} else if (event.key === "Enter" || event.key === " ") {
					event.preventDefault();
					jump(item);
				}
			};
			if (!preferences.enabled || !favoritesOnly && located.length === 0 && !hasMore) return null;
			return /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
					...
					children: searchResultsList.map((item, index) => /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("button", {
						...
						type: "button",
							...
							/* @__PURE__ */ (0, react_jsx_runtime.jsx)("div", {
								...
								children: railItems.map(({ item, anchor, unloaded }, index) => {
									...
									return /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
										key: item.id,
										className: TurnNavigation_module_css_default.markerSlot,
										style: {
											position: "absolute",
											left: 0,
											right: 0,
											top: `${railStartOffset + index * railGap}px`,
											height: `${railGap}px`
										},
										onMouseEnter: () => {
										...
										onFocus: (event) => {
											if (event.currentTarget.contains(event.target)) setFocusedId(item.id);
										},
										onBlur: (event) => {
										...
										children: [/* @__PURE__ */ (0, react_jsx_runtime.jsx)("div", {
											...
											tabIndex: (focusedId ?? activeId ?? railItems[0]?.item.id) === item.id ? 0 : -1,
											onClick: (event) => {
												jump(item);
							...
							jumpStage !== null && /* @__PURE__ */ (0, react_jsx_runtime.jsx)("div", {
								className: "dsh-tl-jumpStatus",
								role: "status",
								children: jumpStatusText(jumpStage, pendingJumpId, t)
							})
						]
					}),
		...
		*  `leftOffset`/`centerOffset` place the rail (horizontal distance from the
		*  panel's left edge, vertical offset from its center), and `markerSpacing`
		...
		*/
		...
		const zh = {
			...
			"timeline.jump.paging": "正在加载第 {turn} 轮…",
			"timeline.jump.locating": "正在定位第 {turn} 轮…",
			"timeline.jump.done": "已定位第 {turn} 轮",
			"timeline.jump.failed": "定位失败，请再点一次",
			...
			"settings.description": "在 Chat 正文左侧显示按用户提问定位的导航轨道。",
			"settings.enabled": "启用",
			"settings.leftOffset": "距左侧距离",
			"settings.centerOffset": "向中部偏移",
			"settings.markerSpacing": "标记间距",
			"settings.recentTurns": "显示轮次数量",
			"settings.recentTurnsHint": "左侧轨道显示的最近轮次条数（5–50 条，默认 25）。",
			"settings.leftOffsetHint": "时间线距会话栏左边缘的距离（0 = 贴合左缘，正值向右移入面板）。",
		...
		const en = {
			...
			"timeline.jump.paging": "Loading turn {turn}…",
			"timeline.jump.locating": "Locating turn {turn}…",
			"timeline.jump.done": "Landed on turn {turn}",
			"timeline.jump.failed": "Could not locate; try again",
			...
			"settings.leftOffset": "Left offset",
			"settings.centerOffset": "Center offset",
			"settings.markerSpacing": "Marker spacing",
			"settings.recentTurns": "Turns shown",
			"settings.recentTurnsHint": "How many recent Turns the left rail lists (5–50, default 25).",
			"settings.leftOffsetHint": "Horizontal distance of the timeline from the panel's left edge (0 = flush with the edge, positive moves right into the panel).",
