window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		*/
		function useChatNavigationBridge({ listRef, columnRef, hasMore, loadingOlder, loadOlder, toBottom }) {
		...
		*/
		...
		if (typeof document !== "undefined" && document.querySelector("style[data-plugin-css=" + JSON.stringify(tagId$1) + "]") === null) {
			...
			tag.textContent = css$1 + ".B_rMOG_navigationSeat{width:100%;margin:0}.RhpIHW_rail{left:-4px}.RhpIHW_desktopTrigger{left:0}.RhpIHW_track{display:flex;flex-direction:column;justify-content:center}.RhpIHW_track:before{display:none}.RhpIHW_rail .RhpIHW_track{left:calc(var(--turn-nav-left, 0px) + 4px);transform:translateY(var(--turn-nav-center, 0px))}.RhpIHW_marker{position:relative;top:auto;left:0;height:var(--turn-nav-spacing, 15px);min-height:3px;max-height:var(--turn-nav-spacing, 15px);flex:0 1 var(--turn-nav-spacing, 15px)}.RhpIHW_mark{top:50%;left:12px;right:auto;width:9px;height:3px;opacity:.32;transform:translateY(-50%);transform-origin:left center}.RhpIHW_active .RhpIHW_mark{width:9px;height:3px;transform:translateY(-50%);background:var(--dsw-alias-label-primary);opacity:.55}.RhpIHW_marker:hover .RhpIHW_mark{width:39px;background:var(--dsw-alias-label-primary);opacity:.9}.RhpIHW_marker:has(+ .RhpIHW_marker:hover) .RhpIHW_mark,.RhpIHW_marker:hover + .RhpIHW_marker .RhpIHW_mark{width:30px}.RhpIHW_marker:has(+ .RhpIHW_marker + .RhpIHW_marker:hover) .RhpIHW_mark,.RhpIHW_marker:hover + .RhpIHW_marker + .RhpIHW_marker .RhpIHW_mark{width:21px}.RhpIHW_marker:has(+ .RhpIHW_marker + .RhpIHW_marker + .RhpIHW_marker:hover) .RhpIHW_mark,.RhpIHW_marker:hover + .RhpIHW_marker + .RhpIHW_marker + .RhpIHW_marker .RhpIHW_mark{width:15px}.RhpIHW_marker:focus-visible .RhpIHW_mark{width:9px}.RhpIHW_rail:hover .RhpIHW_railClose{opacity:0}.RhpIHW_railClose:focus-visible{opacity:1}";
				document.head.appendChild(tag);
			}
		...
		function TurnNavigation({ useSession, usePreferences, layout, hasMore, loadingOlder, scrollToAnchor, trackAnchors, loadOlder, t }) {
			...
			const items = (0, react.useMemo)(() => buildTurnNavigationIndex({
				timeline,
				locations,
				nodes
			}, pending, running), [
			...
			const autoHidden = items.length < 3 && !hasMore;
			const positions = (0, react.useMemo)(() => new Map(layout.anchors.map((anchor) => [anchor.key, anchor])), [layout.anchors]);
			const located = (0, react.useMemo)(() => items.flatMap((item) => {
				const anchor = positions.get(item.anchorKey);
			...
			const viewportActive = (0, react.useMemo)(() => {
				...
				let active = located.find((entry) => entry.anchor.intersecting) ?? located[0];
				for (const entry of located) if (entry.anchor.position <= layout.readingPosition) active = entry;
				else break;
			...
			const searchResults = (0, react.useMemo)(() => normalizedSearchQuery === "" ? [] : items.filter((item) => item.searchText.includes(normalizedSearchQuery)), [items, normalizedSearchQuery]);
			...
			(0, react.useEffect)(() => {
				trackAnchors(preferences.enabled && !autoHidden ? items.map((item) => item.anchorKey) : []);
				return () => {
					trackAnchors([]);
				};
			}, [
			...
			(0, react.useEffect)(() => {
				if (manualActive !== null && !items.some((item) => item.id === manualActive)) setManualActive(null);
			}, [items, manualActive]);
			...
			const onMarkerKeyDown = (event, item) => {
				if (event.key === "ArrowUp" || event.key === "ArrowLeft") {
					...
					event.preventDefault();
					const first = items[0];
					if (first !== void 0) moveFocus(first.id, -items.length);
				} else if (event.key === "End") {
					event.preventDefault();
					const last = items.at(-1);
					if (last !== void 0) moveFocus(last.id, items.length);
				} else if (event.key === "Enter" || event.key === " ") {
					event.preventDefault();
					jump(item);
				}
			};
			if (!preferences.enabled || autoHidden || located.length === 0 && !hasMore) return null;
			return /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
						...
						type: "button",
							...
							/* @__PURE__ */ (0, react_jsx_runtime.jsx)("div", {
								...
								children: located.map(({ item, anchor }, index) => {
									...
									const sparseMinor = preferences.density === "sparse" && index % 4 !== 0 && !active;
									return /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("button", {
										ref: (element) => {
										...
										type: "button",
										className: clsx(TurnNavigation_module_css_default.marker, active && TurnNavigation_module_css_default.active, sparseMinor && TurnNavigation_module_css_default.minor),
										style: { "--turn-position": anchor.position },
										"aria-label": markerLabel(item, t),
										"aria-current": current ? "location" : void 0,
										tabIndex: (focusedId ?? activeId ?? items[0]?.id) === item.id ? 0 : -1,
										onClick: (event) => {
											jump(item);
					...
					drawerOpen && /* @__PURE__ */ (0, react_jsx_runtime.jsx)("div", {
						...
						children: /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("section", {
							...
							children: [
								...
								}), items.length === 0 && normalizedSearchQuery === "" && /* @__PURE__ */ (0, react_jsx_runtime.jsx)("div", {
								...
								children: (normalizedSearchQuery === "" ? items : searchResults).map((item) => {
									const metrics = performanceParts(item, t);
									return /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("button", {
										type: "button",
		...
		*/
		var TurnNavigationPreferences = class {
			...
			adopt(value) {
				...
				if (next.enabled === this.snapshot.enabled && next.defaultVisible === this.snapshot.defaultVisible && next.density === this.snapshot.density && next.followHighlight === this.snapshot.followHighlight && next.leftOffset === this.snapshot.leftOffset && next.centerOffset === this.snapshot.centerOffset && next.markerSpacing === this.snapshot.markerSpacing) return;
				this.snapshot = next;
				this.publish();
			}
			publish() {
