window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		*/
		...
		const ChatNodeSeat = (0, react.memo)(function ChatNodeSeat({ nodeKey, selectedCallId, cwd, openFile, inspectCall, forkAt, loadImage, fileMentions, useSession, renderSlot, t, registerAnchor }) {
		...
		*/
		...
		function apply$1(ctx) {
			...
			ctx.slots.inject("conversation.chat.navigation", () => {
				if (!supportsTurnNavigationSlot(ctx)) {
					ctx.logger.warn("ui-turn-navigation: incompatible conversation.chat.navigation contract; feature disabled");
					return () => {};
				}
				registered = true;
				return ctx.slots.register({
					name: "conversation.chat.navigation",
					locale: NS,
					inject: () => ({
						hooks: { preferences },
						setPreference: (key, value) => {
			...
			queueMicrotask(() => {
				if (!registered && !supportsTurnNavigationSlot(ctx)) ctx.logger.warn("ui-turn-navigation: conversation.chat.navigation is unavailable; waiting for a compatible ui-conversation declaration");
			});
		}
		//#endregion
		//#region src/client/index.ts
		/** Services required by the combined rc.6 conversation browser plugin. */
		const inject = [
			"slots",
			"layout",
			"sessions",
		...
		/** Mount the patched conversation first, then its optional navigation contribution. */
		function apply(ctx) {
			apply$2(ctx);
			apply$1(ctx);
		}
