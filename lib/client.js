	...
	factory: (require) => {
		...
		function useChatNavigationBridge({ sessionId, listRef, columnRef, hasMore, loadingOlder, loadOlder, toBottom, forkAt }) {
			...
			return {
				...
				owner: {
					...
					hasMore,
					loadingOlder,
					scrollToAnchor: (0, react.useCallback)((key, behavior) => {
						const list = listRef.current;
						const element = elements.current.get(key);
						if (list === null || element === void 0) return false;
						const scrollport = navigationScroller(list);
						const top = element.getBoundingClientRect().top - scrollport.getBoundingClientRect().top + scrollport.scrollTop - 16;
						scrollport.scrollTo({
							top: Math.max(0, top),
							behavior
						});
		...
		*/
		...
		if (typeof document !== "undefined" && document.querySelector("style[data-plugin-css=" + JSON.stringify(tagId$1) + "]") === null) {
			...
			tag.textContent += ".RhpIHW_railBandSlot{transition:transform .17s cubic-bezier(.22,.75,.18,1);will-change:transform}.RhpIHW_railPreviewSlot{pointer-events:none;opacity:0;z-index:0;transition:transform .17s cubic-bezier(.22,.75,.18,1),opacity .09s ease-in}.RhpIHW_track:hover .RhpIHW_railPreviewSlot,.RhpIHW_track:focus-within .RhpIHW_railPreviewSlot{opacity:1;transition-duration:.17s,.14s;transition-timing-function:cubic-bezier(.22,.75,.18,1),ease-out}.RhpIHW_railPeekMark{background:var(--dsw-alias-label-tertiary);border-radius:2px;position:absolute;top:50%;transform:translateY(-50%);transition:opacity .12s ease-out}.RhpIHW_railPreviewSlot[data-turn-nav-peek-depth=\"1\"] .RhpIHW_railPeekMark{width:7px;height:2px;left:13px;opacity:.42}.RhpIHW_railPreviewSlot[data-turn-nav-peek-depth=\"2\"] .RhpIHW_railPeekMark{width:5px;height:2px;left:14px;opacity:.24}.RhpIHW_track{z-index:1}.RhpIHW_track[data-turn-nav-motion=\"burst\"] .RhpIHW_railBandSlot{transition-duration:.11s;transition-timing-function:cubic-bezier(.16,.84,.24,1)}.RhpIHW_track[data-turn-nav-motion=\"burst\"] .RhpIHW_railPreviewSlot{transition-property:transform,opacity;transition-duration:.11s,.09s;transition-timing-function:cubic-bezier(.16,.84,.24,1),ease-in}.RhpIHW_track[data-turn-nav-motion=\"burst\"]:hover .RhpIHW_railPreviewSlot,.RhpIHW_track[data-turn-nav-motion=\"burst\"]:focus-within .RhpIHW_railPreviewSlot{transition-duration:.11s,.14s;transition-timing-function:cubic-bezier(.16,.84,.24,1),ease-out}.RhpIHW_markerSlot{min-height:0}.RhpIHW_markerSlot:hover,.RhpIHW_markerSlot:focus-within,.RhpIHW_markerSlot[data-turn-nav-disclosed=true]{z-index:4}.RhpIHW_marker{height:100%;min-height:0}.RhpIHW_mark{pointer-events:none}@media (prefers-reduced-motion:reduce){.RhpIHW_railBandSlot,.RhpIHW_railPeekMark{transition:none}}@media (forced-colors:active){.RhpIHW_railPeekMark{background:CanvasText}}";
			...
			if (typeof document !== "undefined" && document.querySelector("style[data-plugin-css=" + JSON.stringify(surfaceTagId) + "]") === null) {
				...
				surfaceTag.textContent += ".RhpIHW_tooltip{left:calc(100% + 24px)}.RhpIHW_markerSlot::after{width:24px}";
				...
				surfaceTag.textContent += ".RhpIHW_host[data-turn-nav-side=\"right\"] .RhpIHW_tooltip{left:auto;right:calc(100% + 24px)}.RhpIHW_host[data-turn-nav-side=\"right\"] .RhpIHW_markerSlot::after{left:auto;right:100%}.RhpIHW_host[data-turn-nav-side=\"right\"] :is(.RhpIHW_earlier,.RhpIHW_searchTrigger,.RhpIHW_favoriteTrigger){left:auto;right:7px}.RhpIHW_host[data-turn-nav-side=\"right\"] .RhpIHW_railClose{left:auto;right:28px}.RhpIHW_host[data-turn-nav-side=\"right\"] .RhpIHW_searchPanel{left:auto;right:42px}";
				surfaceTag.textContent += ".RhpIHW_markerSlot::after{pointer-events:none}.RhpIHW_markerSlot[data-turn-nav-disclosed=true]::after,.RhpIHW_markerSlot:focus-within::after{pointer-events:auto}";
		...
		function TurnNavigation({ sessionId, useSession, usePreferences, layout, hasMore, loadingOlder, scrollToAnchor, trackAnchors, loadOlder, forkAt, setPreference, t }) {
			...
			// Fixed geometry: the rail and the trigger stack never shift when the
			// load-earlier button appears/disappears.
			const railTrackTop = 76;
			...
			const unloadedCount = (0, react.useMemo)(() => {
				if (remoteIndex === null) return 0;
				const loadedIds = new Set(items.map((item) => item.id));
				return remoteIndex.items.filter((hit) => !loadedIds.has(`turn:${hit.turn}`)).length;
			}, [remoteIndex, items]);
			const earlierLabel = loadingOlder
				? t("timeline.loadingEarlier")
				: unloadedCount > 0
					? t("timeline.earlierCount", { count: unloadedCount })
					: t("timeline.earlier");
			...
			const railTabStopId = railItems.some(({ item }) => item.id === focusedId) ? focusedId : railItems.some(({ item }) => item.id === activeId) ? activeId : railItems[0]?.item.id;
			...
			const [pendingJumpId, setPendingJumpId] = (0, react.useState)(null);
			const [jumpMeasureTick, setJumpMeasureTick] = (0, react.useState)(0);
			const jumpAttemptsRef = (0, react.useRef)(0);
			const jumpPagesRef = (0, react.useRef)(0);
			(0, react.useEffect)(() => {
				if (pendingJumpId === null) return;
				const item = allItems.find((candidate) => candidate.id === pendingJumpId);
				if (item === void 0) {
					console.warn("dsh-codex-timeline: jump target", pendingJumpId, "is not in the session index");
					setPendingJumpId(null);
					return;
				}
				if (item.anchorKey !== void 0) {
					// The Turn materialized. Never page on past the target (that
					// used to load the WHOLE remaining history); once its anchor
					// is measured, scroll to it and settle.
					if (positions.has(item.anchorKey)) {
						const landed = scrollToAnchor(item.anchorKey, prefersReducedMotion() ? "auto" : "smooth");
						console.log("dsh-codex-timeline: jump landed", item.id, landed);
						setPendingJumpId(null);
						return;
					}
					// Just-prepended rows can report zero height for one frame
					// and be skipped by the measure. Retry re-measuring briefly;
				...
				if (jumpPagesRef.current >= 30) {
					jumpPagesRef.current = 0;
					console.warn("dsh-codex-timeline: jump gave up after 30 pages", item.id);
					setPendingJumpId(null);
					return;
				}
				jumpPagesRef.current += 1;
				if (!loadingOlder) Promise.resolve(loadOlder()).catch((error) => {
					console.error("dsh-codex-timeline: failed to materialize selected history turn:", error);
					setPendingJumpId(null);
				});
			}, [pendingJumpId, allItems, positions, hasMore, loadingOlder, loadOlder, scrollToAnchor, trackAnchors, shouldRenderRail, visibleItems, jumpMeasureTick]);
			const jump = (0, react.useCallback)((item) => {
				setManualActive(item.id);
				jumpAttemptsRef.current = 0;
				jumpPagesRef.current = 0;
				if (item.anchorKey !== void 0 && positions.has(item.anchorKey)) scrollToAnchor(item.anchorKey, prefersReducedMotion() ? "auto" : "smooth");
				else setPendingJumpId(item.id);
				setDrawerOpen(false);
			}, [positions, scrollToAnchor]);
					...
					railVisible && /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("nav", {
						...
						children: [
							...
							hasMore && /* @__PURE__ */ (0, react_jsx_runtime.jsx)("button", {
									type: "button",
									className: TurnNavigation_module_css_default.earlier,
									style: { top: 52 },
									disabled: loadingOlder,
						...
						type: "button",
							...
							/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
								...
								children: railBandItems.map(({ entry: { item, unloaded }, edge, depth, windowIndex, relativeIndex }) => {
									...
									if (edge !== void 0) return /* @__PURE__ */ (0, react_jsx_runtime.jsx)("div", {
										key: item.id,
									...
									return /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
										...
										children: [/* @__PURE__ */ (0, react_jsx_runtime.jsx)("div", {
											...
											"aria-label": unloaded ? [markerLabel(item, t), t("timeline.index.unloaded")].join("，") : markerLabel(item, t),
		...
		*/
		...
		const zh = {
			...
			"timeline.earlierCount": "加载更早（剩余 {count} 轮）",
		...
		const en = {
			...
			"timeline.earlierCount": "Load earlier ({count} turns left)",
		...
		function AdditiveTurnNavigation({ sessionId, useSession, usePreferences, loadOlder, forkAt, setPreference, t }) {
			...
			(0, react.useLayoutEffect)(() => {
				...
				for (const row of list.querySelectorAll("[data-chat-anchor-key]")) {
					...
					if (key === void 0) continue;
					anchors.push(key);
					navigation.registerAnchor(key, row);
				}
				navigation.scheduleMeasure(true);
				setPortalTarget(seat);
				return () => {
					for (const key of anchors) navigation.registerAnchor(key, null);
					listRef.current = null;
					columnRef.current = null;
					seat.remove();
				};
				...
				sessionId,
				order,
				navigation.registerAnchor,
				navigation.scheduleMeasure
			]);
