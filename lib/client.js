	...
	factory: (require) => {
			...
			return {
				...
				owner: {
					...
					scrollToAnchor: (0, react.useCallback)(async (key, options = {}) => {
						...
						if (landedTimerRef.current !== null) window.clearTimeout(landedTimerRef.current);
						landedElementRef.current?.removeAttribute("data-dsh-codex-timeline-landed");
						landedElementRef.current = element;
						element.setAttribute("data-dsh-codex-timeline-landed", "true");
						landedTimerRef.current = window.setTimeout(() => {
							element.removeAttribute("data-dsh-codex-timeline-landed");
							if (landedElementRef.current === element) landedElementRef.current = null;
		...
		*/
		...
		function TurnNavigation({ sessionId, useSession, usePreferences, layout, hasMore, loadingOlder, cancelScroll, manualScrollVersion, materializedAnchorCount, scrollToAnchor, trackAnchors, loadOlder, forkAt, setPreference, t }) {
			...
			}, [jumpRequest, allItems, positions, hasMore, loadingOlder, loadOlder, manualScrollVersion, materializedAnchorCount, order, scrollToAnchor, trackAnchors, shouldRenderRail, visibleItems, finishJump, t, jumpMeasureTick]);
		...
		*  `side` selects the panel edge, `leftOffset`/`centerOffset` place the rail
		*  (distance inward from that edge, vertical offset from its center), and `markerSpacing`
		...
		*/
		var TurnNavigationPreferences = class {
			...
			adopt(value) {
				...
				if (next.enabled === this.snapshot.enabled && next.favorites.length === this.snapshot.favorites.length && next.favorites.every((id, index) => id === this.snapshot.favorites[index]) && next.defaultVisible === this.snapshot.defaultVisible && next.density === this.snapshot.density && next.followHighlight === this.snapshot.followHighlight && next.side === this.snapshot.side && next.leftOffset === this.snapshot.leftOffset && next.centerOffset === this.snapshot.centerOffset && next.markerSpacing === this.snapshot.markerSpacing && next.recentTurns === this.snapshot.recentTurns) return;
				this.snapshot = next;
				this.publish();
			}
			publish() {
