 ...
 * profile configuration (cordis.patch.yml) — the same storage every other plugin uses — rather
 ...
 */
const TIMELINE_SETTINGS_NAMESPACE = "codex-timeline";
...
function searchRouteHandler(getSessionQuery, getSessions, getPersistence) {
  return async (req, res) => {
    ...
    try {
      ...
      } else if (typeof persistence?.readRaw === "function") {
